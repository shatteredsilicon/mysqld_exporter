...
func (ch *MySqlConfigHandler) ReloadConfigFromDSN(dsn string, logger log.Logger) error {
	...
	if dsnCfg, err = mysql.ParseDSN(dsn); err != nil {
		return fmt.Errorf("failed to load config from DSN '%s': %w", dsn, err)
	}

	if strings.HasPrefix(dsnCfg.Net, "tcp") {
		if host, portStr, err = net.SplitHostPort(dsnCfg.Addr); err != nil {
			return fmt.Errorf("failed to split host and port from '%s': %w", dsnCfg.Addr, err)
	...
	if ch.Config != nil && ch.Config.Sections != nil {
		mysqlConfig = ch.Config.Sections["client"]
		if dsnCfg.TLSConfig != "custom" {
			mysqlConfig.SslCa = ""
			mysqlConfig.SslCert = ""
			mysqlConfig.SslKey = ""
		}
	...
	mysqlConfig.User = dsnCfg.User
	mysqlConfig.Password = dsnCfg.Passwd
